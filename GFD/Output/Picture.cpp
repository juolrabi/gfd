...
bool Picture::load(const std::string &file, const bool middle)
{
	...
	for(y=0,i=0; y<m_height; y++)
	{
		for(x=0; x<m_width; x++,i++)
		{
		  ...
		  if(middle) m_pixels[i] = Vector4(double(red) / 128.0 - 1.0, double(green) / 128.0 - 1.0, double(blue) / 128.0 - 1.0, 0.0);
		  else m_pixels[i] = Vector4(double(red) / 256.0, double(green) / 256.0, double(blue) / 256.0, 0.0);
		  //std::cout << double(red) << " " << double(green) << " " << double(blue) << std::endl;
		}
